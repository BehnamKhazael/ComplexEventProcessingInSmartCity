...
public abstract class Controller implements Observer, Runnable, ControllerInterface {
    ...
    final NetworkGraph networkGraphPath;


    final HashMap<NodeAddress, LinkedList<NodeAddress>> results;
    final HashMap<NodeAddress, LinkedList<NodeAddress>> paths;
    ...
    public NodeAddress getSinkAddress(int netID) {
        return  new NodeAddress(String.valueOf(netID) + ".1");
    }
    ...
    Controller(Adapter lower, NetworkGraph networkGraph) {
        ...
        this.networkGraphPath = networkGraph;
        results = new HashMap<>();
        paths = new HashMap<>();
    ...
    public void managePacket(NetworkPacket data) {
        ...
        switch (data.getType()) {
            case SDN_WISE_REPORT:
                ...
                networkGraphPath.updateMap(new ReportPacket(data));
                break;
            case SDN_WISE_DATA:
    ...
    public void run() {
        if (lower.open()) {
            ...
            networkGraphPath.addObserver(this);
            register();
            setupNetwork();
            while (!isStopped) {
    ...
    public final void sendPath(byte netId, NodeAddress destination,
        ...
        OpenPathPacket op = new OpenPathPacket(netId, getSinkAddress(netId), destination);
        op.setPath(path)
                .setNxhop(getSinkAddress(netId));
    ...
    public final void setNodeAddress(byte netId, NodeAddress destination,
        ...
        ConfigNodePacket cp = new ConfigNodePacket(netId, getSinkAddress(netId), destination);
    ...
    public final void setNodeBeaconPeriod(byte netId, NodeAddress destination,
            short period) {
        ConfigTimerPacket cp = new ConfigTimerPacket(netId, getSinkAddress(netId), destination);
    ...
    public final void addAcceptedAddress(byte netId, NodeAddress destination,
            NodeAddress newAddr) {
        ConfigAcceptedIdPacket cp = new ConfigAcceptedIdPacket(netId, getSinkAddress(netId), destination);
    ...
    public final void addRule(byte netId, NodeAddress destination,
        ...
        ResponsePacket rp = new ResponsePacket(netId, getSinkAddress(netId), destination);
        rp.setRule(rule)
                .setNxhop(getSinkAddress(netId));
    ...
    public final void removeRule(byte netId,
        ...
        ConfigRulePacket cp = new ConfigRulePacket(netId, getSinkAddress(netId), destination);
    ...
    public void sendFunction(
        ...
        try {
            ...
            List<NetworkPacket> ll = createPackets(
                    netId, getSinkAddress(netId), dest, getSinkAddress(netId), id, buf);
